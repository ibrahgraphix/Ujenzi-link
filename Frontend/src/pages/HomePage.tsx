...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      const [allListings, allProviders, allCats, allAds, regs] = await Promise.all([
        getListings(),
        getProviders(),
        getCategories(),
        getAdverts(),
  ...
  useEffect(() => {
    if (selectedRegion) {
      getDistrictsByRegion(selectedRegion).then(setDistrictsList);
    } else {
      setDistrictsList([]);
    }
  }, [selectedRegion]);
        ...
        <div className="relative max-w-5xl mx-auto text-center space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-3xl mx-auto pt-6 text-center">
            <div className="p-3 rounded-2xl bg-white/5 backdrop-blur-xs border border-white/10">
              <div className="text-xl sm:text-2xl font-black text-white font-heading">Direct</div>
            ...
            <div className="p-3 rounded-2xl bg-white/5 backdrop-blur-xs border border-white/10">
              <div className="text-xl sm:text-2xl font-black text-white font-heading">National</div>
              <div className="text-xs text-blue-200 font-medium">Tanzania Coverage</div>
            </div>
            <div className="p-3 rounded-2xl bg-white/5 backdrop-blur-xs border border-white/10">
